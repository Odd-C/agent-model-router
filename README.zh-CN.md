...
从「原始任务 + 意图」到「可推荐的模型」，完整决策管线一张图看懂：

```text
           任务（task_type / priority / deadline）
                     +  意图（「要便宜点的」）
                              │
                              ▼
   ┌──────────────────────────────────────────────────┐
   ...
   ┌──────────────────────────────────────────────────┐
   │ 硬约束先砍后评（v0.4+）                            │
   │ 不可谈判：cost 上限 / quota 耗尽 / cooldown /       │
   │ deadline 不可行 / health 红线 / 能力上下限          │
   │ （max_latency_ms、min_quality_tier、               │
   │   min_capability_pct）                             │
   └─────────────────────────┬────────────────────────┘
                             │ 过滤后的候选集
                             ▼
   ┌──────────────────────────────────────────────────┐
   │ 六维 Utility 评分（候选集内 min-max 归一化后加权）  │
   │ 质量 / 成本 / 延迟 / 健康 / 额度 / 截止            │
   └─────────────────────────┬────────────────────────┘
                             │ 最高分
                             ▼
      {model, provider, score, breakdown, why}   ← 可解释
```
